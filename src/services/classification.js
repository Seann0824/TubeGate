(function (root) {
  ...
  const D = root.TubeGateDecision || require('../core/decision.js');
  const RequestQueue = root.TubeGateRequestQueue || require('./request-queue.js');

  function createClassificationService({ runtime, classifier, writeStats }) {
    if (!classifier?.id || typeof classifier.classify !== 'function') throw new TypeError('Classifier requires id and classify');
    ...
    async function updateCache(operation) {
      const previous = cacheGate;
      let release;
      cacheGate = new Promise((resolve) => { release = resolve; });
      await previous;
    ...
    async function classify(payload) {
      ...
      let content;
      try { content = C.normalizeContent(payload?.content); }
      catch (_) { return { ok: false, error: 'INVALID_CONTENT' }; }
      ...
      const cached = stored[E.CACHE_KEY]?.[key];
      if (!payload.force && cached && Date.now() - cached.createdAt < config.cacheTtlHours * 3600000) {
        try {
          const decision = D.decide(cached.scores, rules);
          await writeStats({ cacheHitsDelta: 1 });
          return { ok: true, ...decision, cache: 'hit' };
        } catch (_) { /* Ignore an obsolete or corrupt cache entry; classify afresh. */ }
      }
      ...
      return queue.enqueue(requestKey, async () => {
        ...
        if (!latest.enabled || E.stableStringify(latest) !== E.stableStringify(config)) return { ok: false, error: 'CONFIG_CHANGED' };
        if (!await reserveRequest(latest.dailyLimit)) return { ok: false, error: 'DAILY_LIMIT_REACHED' };
        try {
          const result = await classifier.classify({ apiKey: config.typesafeApiKey, model: config.model, content, rules });
          const decision = D.decide(result.scores, rules);
          await updateCache(async () => {
            if (epoch !== cacheEpoch) return;
            const current = await E.storageGet(runtime, 'session', E.CACHE_KEY);
            const entries = Object.entries({ ...(current[E.CACHE_KEY] || {}), [key]: { createdAt: Date.now(), scores: decision.results } });
            await E.storageSet(runtime, 'session', { [E.CACHE_KEY]: Object.fromEntries(entries.slice(-500)) });
          });
          await writeStats({ latencyMs: result.latencyMs, lastError: '' });
          E.debugLog(config, 'classified', { source: content.source, contentId: content.id, contentHash: key, latencyMs: result.latencyMs });
  ...
  root.TubeGateClassification = { createClassificationService };
  if (typeof module !== 'undefined' && module.exports) module.exports = root.TubeGateClassification;
})(globalThis);

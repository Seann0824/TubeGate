import type { Config, ClassificationResult, ContentItem, ContentAdapter } from './types';
import { send } from './shared/messages';
import './adapters/youtube';
import { registry } from './core/adapters';
import * as E from './shared/core';
import * as C from './core/content';
const resolvedAdapter = registry.resolve(location.href);
if (resolvedAdapter) start(resolvedAdapter);
type LocalItem = ContentItem & { key: string; identity: string };
type Entry = {
  card: HTMLElement;
  item: LocalItem;
...
type ViewResult =
  | { pending: true; ok?: false; error?: undefined; shouldHide?: false }
  | (ClassificationResult & { pending?: false });
function start(adapter: ContentAdapter) {
  ...
  let status: { root: HTMLElement; button: HTMLButtonElement } | undefined;
  ...
  function readContent(card: HTMLElement) {
    try {
  ...
  function inScope(card: HTMLElement) {
    if (!context || navigating || !card.isConnected) return false;
    try {
      return adapter.isInScope(card, context);
    } catch (_) {
      return false;
    }
  }
  ...
  function renderStatus() {
    ...
    const hidden = Array.from(entries.values()).filter((entry) =>
      entry.card.classList.contains('smg-blocked')
    ).length;
    const errors = values.filter((value) => value && !value.pending && !value.ok).length;
    status.button.textContent = !config?.enabled
      ? `${itemLabel}过滤已暂停`
      : !hasKey
        ? `${itemLabel}过滤 · 需要 API Key`
        : !E.activeRules(config).length
          ? `${itemLabel}过滤 · 没有启用的规则`
          : `${itemLabel}过滤 · 已检查 ${values.filter((value) => !value.pending).length} · 隐藏 ${hidden}${active ? ' · 检查中' : ''}${errors ? ` · 失败 ${errors}` : ''}`;
  ...
  function renderEntry(entry: Entry) {
    clear(entry);
    if (!config || !allowed() || !inScope(entry.card)) return;
    ...
    if (result?.ok && result.shouldHide && !isRevealed) {
      ...
      const placeholder = node(
        'div',
        `smg-placeholder${config.showPlaceholder ? '' : ' smg-compact'}`
      );
      ...
      const details = node(
        ...
        result.matches
          .map(
            (match) =>
              `${match.name}${config?.showConfidence ? ` · ${Math.round(match.probability * 100)}%` : ''}`
          )
          .join(' / ')
      );
    ...
    button.title =
      (result && !result.ok ? result.error : '') || `按当前过滤规则检查这条${itemLabel}`;
    button.addEventListener('click', (event) => {
      event.preventDefault();
      event.stopPropagation();
      if (active >= (config?.maxConcurrency || 3)) return;
  ...
  function nearViewport(card: HTMLElement) {
    const rect = card.getBoundingClientRect();
    return (
      rect.width > 0 && rect.height > 0 && rect.bottom >= -300 && rect.top <= innerHeight + 600
    );
  }

  async function inspect(entry: Entry, force = false) {
    if (!config || !allowed() || !inScope(entry.card)) return;
    ...
    const response = await send(E.MESSAGE.CLASSIFY_CONTENT, {
      content: C.normalizeContent(entry.item),
      force,
    });
    active -= 1;
  ...
  function scan() {
    ...
    if (nextContext?.key !== context?.key) {
      context = nextContext;
      forgetAll();
    }
    ...
    for (const card of adapter.collect(document, context)) {
      ...
      if (
        ...
        (entry.placeholder && !card.contains(entry.placeholder)) ||
        (entry.controls && !card.contains(entry.controls))
      ) {
      ...
      if (
        !allowed() ||
        revealed.has(item.identity) ||
        results.has(item.key) ||
        active >= (config?.maxConcurrency || 3) ||
        !nearViewport(card)
      )
        continue;
      void inspect(entry);
  ...
  async function refresh() {
    ...
    if (JSON.stringify(config) === JSON.stringify(response.config) && hasKey === response.hasApiKey)
      return;
  ...
  button.addEventListener('click', () => {
    void send(E.MESSAGE.OPEN_SETTINGS);
  });
  root.append(button);
  document.body.append(root);
  status = { root, button };
  renderStatus();
  const own = (target: Node) =>
    (target instanceof Element ? target : target.parentElement)?.closest(OWN);
      ...
      mutations.some(
        (mutation) =>
          ...
          (mutation.type !== 'childList' ||
            [...mutation.addedNodes, ...mutation.removedNodes].some((item) => !own(item)))
      )
    )
      scheduleScan();
  });
  observer.observe(document.documentElement, {
    ...
    characterData: true,
    attributes: true,
    attributeFilter: adapter.observedAttributes || ['href', 'hidden'],
  });
  for (const event of adapter.navigation?.start || [])
    document.addEventListener(event, () => {
      navigating = true;
      forgetAll();
    });
  for (const event of adapter.navigation?.finish || [])
    document.addEventListener(event, () => {
      navigating = false;
      routeChanged();
      renderStatus();

...
import * as Y from './urls';
import { registry } from '../../core/adapters';
const CARD =
  'ytd-rich-item-renderer, ytd-video-renderer, ytd-compact-video-renderer, ytd-grid-video-renderer, ytd-reel-item-renderer, yt-lockup-view-model, ytm-shorts-lockup-view-model, ytm-shorts-lockup-view-model-v2';
function extract(card: HTMLElement, context: PageContext | null) {
  ...
  const link =
    links.find(
      (item) =>
        item.matches(
          'a#video-title, a#video-title-link, a.yt-lockup-metadata-view-model__title, a.shortsLockupViewModelHostEndpoint'
        ) && Y.videoIdFromUrl(item.href)
  ...
  const titleNode = card.querySelector(
    '#video-title, #video-title-link, .yt-lockup-metadata-view-model__title, .shortsLockupViewModelHostMetadataTitle, .shortsLockupViewModelHostMetadataTitleV2, h3'
  );
  const title = E.normalizeText(
    titleNode?.getAttribute('title') || titleNode?.textContent || link.getAttribute('title')
  );
  if (!title) return null;
  const channelNode = card.querySelector(
    '#channel-name a, #channel-name, #byline-container a, a[href^="/@"], a[href^="/channel/"], a[href^="/c/"]'
  );
  const descriptionNode = card.querySelector(
    '#description-text, .metadata-snippet-text, #description'
  );
  return C.normalizeContent({
    source: 'youtube',
    id: videoId,
    type: 'video',
    title,
    author: channelNode?.textContent || '',
    text: descriptionNode?.textContent || '',
  });
}

function isInScope(card: HTMLElement, context: PageContext | null) {
  if (!context || !card.isConnected) return false;
  if (
    card.closest(
      'ytd-ad-slot-renderer, ytd-promoted-video-renderer, ytd-display-ad-renderer, [is-ad]'
    )
  )
    return false;
...
export const youtubeAdapter = registry.register({
  id: 'youtube',
  itemLabel: '视频',
  matches(url) {
    try {

import { createArea, success, failure } from './helpers';
import type { MessageType, MessagePayloads, MessageResponse } from '../src/types';
import type { Questions } from '../src/providers/jev-protocol';
import { record } from '../src/types';
type ApiBody = { state: { content: string }; questions: Questions };
type RequestOptions = { body: string; headers: Record<string, string> };
type Listener = (message: unknown, sender: object, respond: (response: unknown) => void) => void;
...
const backgroundBundle = buildSync({
...
function content(title: string, id = 'local-id') {
  return { source: 'youtube', id, type: 'video', title, author: '', text: '' };
}
...
function createBackgroundHarness() {
  ...
  let installedListener!: (details: { reason: string }) => Promise<void>;
  let startupListener!: () => Promise<void>;
  const requests: { url: string; options: RequestOptions; body: ApiBody }[] = [];
  ...
  const chrome = {
    ...
    runtime: {
      ...
      onInstalled: {
        addListener(listener: typeof installedListener) {
          installedListener = listener;
        },
      },
      onStartup: {
        addListener(listener: typeof startupListener) {
  ...
  vm.runInContext(backgroundBundle, sandbox);
  assert.equal(typeof messageListener, 'function');
  assert.equal(typeof installedListener, 'function');
  assert.equal(typeof startupListener, 'function');
  const dispatch = <K extends MessageType>(message: {
    type: K;
    payload?: MessagePayloads[K];
  }): Promise<MessageResponse<K>> =>
    new Promise((resolve) =>
      messageListener(message, {}, (response) => resolve(response as MessageResponse<K>))
    );
  return {
    dispatch,
    requests,
...
test('background pipeline keeps the API key out of the request body, caches duplicates, and fails open', async () => {
  ...
  const first = await harness.dispatch({
    type: E.MESSAGE.CLASSIFY_CONTENT,
    payload: { content: content('hide-me', 'reply-1') },
  });
  ...
  const cached = await harness.dispatch({
    type: E.MESSAGE.CLASSIFY_CONTENT,
    payload: { content: content('hide-me', 'reply-2') },
  });
  assert.equal(cached.ok, true);
  assert.equal(success(cached).cache, 'hit');
  assert.equal(harness.requests.length, 1);

  const rechecked = await harness.dispatch({
    type: E.MESSAGE.CLASSIFY_CONTENT,
    payload: { content: content('hide-me', 'reply-2'), force: true },
  });
  assert.equal(rechecked.ok, true);
  assert.equal(success(rechecked).cache, 'miss');
  assert.equal(harness.requests.length, 2);

  await harness.dispatch({ type: E.MESSAGE.SAVE_CONFIG, payload: { dailyLimit: 1 } });
  const limited = await harness.dispatch({
    type: E.MESSAGE.CLASSIFY_CONTENT,
    payload: { content: content('another safe video', 'reply-3') },
  });
  assert.equal(limited.ok, false);
  assert.equal(failure(limited).error, 'DAILY_LIMIT_REACHED');

  await harness.dispatch({ type: E.MESSAGE.SAVE_CONFIG, payload: { dailyLimit: 2000 } });
  const failed = await harness.dispatch({
    type: E.MESSAGE.CLASSIFY_CONTENT,
    payload: { content: content('network-fail', 'reply-4') },
  });
  assert.equal(failed.ok, false);
  assert.equal(failure(failed).error, 'API_UNAVAILABLE');
  assert.equal(record(failed).shouldHide, undefined);
});
...
test('background queue never exceeds the configured concurrency', async () => {
  ...
  const results = await Promise.all(
    Array.from({ length: 7 }, (_, index) =>
      harness.dispatch({
        type: E.MESSAGE.CLASSIFY_CONTENT,
        payload: { content: content(`safe-${index}`, `reply-${index}`) },
      })
    )
  );
  assert.ok(results.every((result) => result.ok));
  assert.ok(
    harness.maxActiveRequests <= 2,
    `observed ${harness.maxActiveRequests} concurrent requests`
  );
});
...
test('video totals accumulate across pages independently from page metrics', async () => {
  ...
  await harness.dispatch({
    type: E.MESSAGE.RECORD_CONTENT_EVENT,
    payload: { action: 'checked', page: { checked: 1, hidden: 0 } },
  });
  await harness.dispatch({
    type: E.MESSAGE.RECORD_CONTENT_EVENT,
    payload: { action: 'hidden', page: { checked: 1, hidden: 1 } },
  });
  const state = await harness.dispatch({ type: E.MESSAGE.GET_STATE });
  assert.equal(success(state).stats.totalChecked, 1);
  assert.equal(success(state).stats.totalHidden, 1);
  assert.equal(success(state).stats.pageChecked, 1);
  assert.equal(success(state).stats.pageHidden, 1);
});
...
test('cached probabilities are re-evaluated after threshold edits without another API call', async () => {
  ...
  const first = await harness.dispatch({
    type: E.MESSAGE.CLASSIFY_CONTENT,
    payload: { content: content('hide-me') },
  });
  assert.equal(success(first).shouldHide, true);
  ...
  assert.equal(success(second).cache, 'hit');
  assert.equal(success(second).shouldHide, false);
  assert.equal(harness.requests.length, 1);
});
...
test('concurrent requests consume the exact daily limit without double-counting reservations', async () => {
  ...
  await harness.dispatch({ type: E.MESSAGE.SAVE_API_KEY, payload: { apiKey: 'apikey_secret' } });
  await harness.dispatch({
    type: E.MESSAGE.SAVE_CONFIG,
    payload: { dailyLimit: 3, maxConcurrency: 3 },
  });
  const responses = await Promise.all(
    Array.from({ length: 6 }, (_, index) =>
      harness.dispatch({
        type: E.MESSAGE.CLASSIFY_CONTENT,
        payload: { content: content(`unique-${index}`) },
      })
    )
  );
  ...
  assert.equal(
    responses.filter((response) => !response.ok && response.error === 'DAILY_LIMIT_REACHED').length,
    3
  );
});
...
test('simultaneous duplicates share a request and disabled protection sends no content', async () => {
  ...
  const missing = await harness.dispatch({
    type: E.MESSAGE.CLASSIFY_CONTENT,
    payload: { content: content('safe') },
  });
  assert.equal(failure(missing).error, 'API_KEY_MISSING');
  await harness.dispatch({ type: E.MESSAGE.SAVE_API_KEY, payload: { apiKey: 'apikey_secret' } });
  await Promise.all(
    Array.from({ length: 4 }, () =>
      harness.dispatch({
        type: E.MESSAGE.CLASSIFY_CONTENT,
        payload: { content: content('same-title') },
      })
    )
  );
  assert.equal(harness.requests.length, 1);
  ...
  assert.equal(success(disabled).shouldHide, false);
  assert.equal(harness.requests.length, 1);
});
...
test('malformed successful responses fail open and are not cached as safe decisions', async () => {
  ...
  const response = await harness.dispatch({
    type: E.MESSAGE.CLASSIFY_CONTENT,
    payload: { content: content('malformed-response') },
  });
  assert.equal(response.ok, false);
  assert.equal(failure(response).error, 'API_INVALID_RESPONSE');
  assert.equal(Object.keys(record(harness.session.values[E.CACHE_KEY])).length, 0);
});

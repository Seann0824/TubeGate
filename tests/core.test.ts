...
import * as Y from '../src/adapters/youtube/urls';
import * as C from '../src/core/content';
import * as D from '../src/core/decision';
import * as P from '../src/providers/jev-protocol';
...
test('only supported YouTube pages are eligible; player timestamps do not reset the route', () => {
  ...
  assert.notEqual(
    Y.pageContext('https://www.youtube.com/results?search_query=cats')?.key,
    Y.pageContext('https://www.youtube.com/results?search_query=dogs')?.key
  );
  assert.equal(
    Y.pageContext('https://www.youtube.com/watch?v=dQw4w9WgXcQ&t=12')?.key,
    Y.pageContext('https://www.youtube.com/watch?v=dQw4w9WgXcQ&t=90')?.key
  );
  for (const path of [
    '/feed/subscriptions',
    '/feed/history',
    ...
    assert.equal(Y.pageContext(`https://www.youtube.com${path}`), null);
...
test('video identities accept watch and Shorts cards, reject playlists and external links', () => {
  for (const url of ['/watch?v=dQw4w9WgXcQ&list=example', '/shorts/dQw4w9WgXcQ'])
    assert.equal(Y.videoIdFromUrl(url), 'dQw4w9WgXcQ');
  for (const url of [
...
test('rules use individual thresholds and do not inherit comment emoji overrides', () => {
  ...
  const response = {
    answers: {
      sexual_content: { pTrue: 0.84 },
      sexual_solicitation: { noul: 0.5 },
      spam_behavior: { probabilities: { true: 0.91 } },
    },
  };
  const decision = D.decide(P.readScores(response, rules), rules);
  assert.deepEqual(
    decision.matches.map((item) => item.ruleId),
    ['spam_behavior']
  );
  assert.equal(
    D.decide(
  ...
  for (const malformed of [
    null,
    {},
    { answers: { sexual_content: { noul: '1' } } },
    { answers: { sexual_content: { noul: 3 } } },
  ]) {
    assert.throws(() => D.decide(P.readScores(malformed, rules), rules), {
...
test('cache identity tracks wording and model but allows reusing scores after threshold changes', async () => {
  ...
  assert.equal(
    ...
    await E.cacheKey(
      'video',
      rules.map((rule) => ({ ...rule, threshold: 0.99 })),
      'model'
    )
  );
  ...
  assert.notEqual(
    ...
    await E.cacheKey(
      'video',
      rules.map((rule) => ({ ...rule, instructions: 'changed' })),
      'model'
    )
  );
});

...
src/adapters/example/
  urls.js       # 纯 URL 解析和页面上下文，可做非 UI 单元测试
  index.js      # DOM 选择器、提取、范围与注册
  styles.css    # 可选的网站专属样式
```

参考 `src/adapters/youtube/`。在 `index.js` 中调用 `TubeGateAdapters.register()`，实现 matches、getContext、collect、extract 和 isInScope。
...
- 站点导航事件和需要观察的属性放进适配器；共享 `content.js` 不增加网站选择器或来源分支。
...
JS 顺序：shared/core → core/content → core/adapters → 来源 urls → 来源 index → content。CSS 顺序：共享 content.css → 来源 styles.css。

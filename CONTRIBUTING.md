...
TubeGate 当前只支持 YouTube，底层通过 Adapter 接口为未来来源保留扩展点。提交新网站支持前，说明适用页面、提取字段、排除区域和最小权限。
...
**不编写 UI 单元测试。** DOM 选择器、布局、按钮和实际页面行为交由用户手动验收，在 PR 中列出改动点、操作步骤和预期结果。未获得实际用户验证，不得声称页面效果通过。
